...
public class MyGridBaseAdapter extends BaseAdapter {
    ...
    public MyGridBaseAdapter(Context context, List<MyBaseEntity> myBaseEntityList) {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        MyGridBaseAdapter.ViewHolder holder = new MyGridBaseAdapter.ViewHolder();
        if(view == null){
            //根据布局文件 生成转换的视图
            view = LayoutInflater.from(context).inflate(R.layout.item_grid_view,null);
            holder.icon = view.findViewById(R.id.grid_base_icon);
            holder.name = view.findViewById(R.id.grid_base_name);
            holder.desc = view.findViewById(R.id.grid_base_desc);
            holder.baseButton = view.findViewById(R.id.grid_base_button);
            //将视图持有者保存到转换视图中
            view.setTag(holder);
        }else{
            holder = (MyGridBaseAdapter.ViewHolder) view.getTag();
        }
